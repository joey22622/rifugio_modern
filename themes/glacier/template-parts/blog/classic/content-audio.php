...
if ( class_exists('acf') ) :
  ...
  // vars ( ACF )
  $audio_host_type = get_field('audio_host_type');
  $audio_embed_code = get_field('audio_embed_code');
  $self_hosted = get_field('audio_self_hosted');
  $self_hosted_title = get_field('audio_self_hosted_title');
  ...
  <article id="post-<?php the_ID(); ?>" <?php post_class($column); ?>>
   ...
   <div class="<?php echo esc_html($blog_animation); ?>">
  	...
  	<div class="entry-header">
         ...
         <?php if ($audio_host_type == 'embed-code') : ?>

          	  <div class="post-intro type-audio">
        	  		   <?php 

                 echo wp_kses( $audio_embed_code,
        ...
        <?php if ($audio_host_type == 'self-hosted') : ?>

           <?php if ($thumbnail_src) : ?>

              <div class="post-intro type-audio image-audio" style="background-image: url('<?php echo esc_url($thumbnail_src[0]); ?>');">

                  <h2><?php echo esc_html($self_hosted_title); ?></h2>
                  <?php echo do_shortcode( '[audio src="'. esc_url($self_hosted) .'"][/audio]' ); ?>
